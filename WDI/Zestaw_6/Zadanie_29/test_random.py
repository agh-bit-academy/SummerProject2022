...
STATIC_TABLE_TESTS = [[COORDS_TAB, randint(0, MAX_R)]
                      for _ in range(TEST_NUM)]

R = randint(0, MAX_R)
STATIC_R_TESTS = [[[RandFixArray(3, L_RANGE, R_RANGE).get()
                    for _ in range(SIZE)], R]
                  for _ in range(TEST_NUM)]

ALL_RANDOM_TESTS = [[[RandFixArray(3, L_RANGE, R_RANGE).get()
                      for _ in range(SIZE)], randint(0, MAX_R)]
                    for _ in range(TEST_NUM)]
