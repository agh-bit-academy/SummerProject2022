...
MIN_PROD = 1
MAX_PROD = 50
TEST_SMALL = [(RandArr(randint(MIN_SIZE, MAX_SIZE), LRANGE, RRANGE).get(), randint(MIN_PROD, MAX_PROD)) for _ in
              range(TEST_NUM)]


@pytest.mark.order(1)
@pytest.mark.dependency(name='test_s6t12_basic', scope='session')
class TestSmall:
